...
// 示例数据
const examples = [
  { symbol: '☰☰', name: '乾', pinyin: 'Qian' },
  { symbol: '☷☷', name: '坤', pinyin: 'Kun' },
  { symbol: '☵☳', name: '屯', pinyin: 'Zhun' },
  { symbol: '☶☵', name: '蒙', pinyin: 'Meng' },
  { symbol: '☲☰', name: '大有', pinyin: 'Dayou' }
];
...
function main() {
  ...
  examples.forEach(example => {
    const svgContent = generateDaxiangSVG(example.symbol, example.name);
    const filename = `${example.pinyin}.svg`;
  ...
  console.log(`\n🎉 Generated ${examples.length} Daxiang SVG files in: ${outputDir}`);
  console.log('\nGenerated files:');
  examples.forEach(example => {
    console.log(`  - ${example.pinyin}.svg (${example.symbol} ${example.name})`);
  });
}
